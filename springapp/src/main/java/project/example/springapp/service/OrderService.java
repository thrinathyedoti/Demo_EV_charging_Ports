...
public class OrderService {
    ...
    public void orderfromproduct(int cusId){
        ...
        for (int i=0;i<customer.getDummyCarts().size();i++) {
            int proId = customer.getDummyCarts().get(i).getProduct().getProductId();
            int quan= customer.getDummyCarts().get(i).getQuantity();
            ...
            Long d_cartId=customer.getDummyCarts().get(i).getDummyCartId();
            dummyCartRepo.deleteById(d_cartId);
        }
    }
    ...
    public int d_cartcount(int cusId){
        ...
        int c=customer1.getDummyCarts().size();
        return c;
    }
